...
while( log10(res_avg) > tol ){
    res_avg=0;
    for( int i=1 ; i < n-1 ; ++i){
        T[i] = -0.5*   (  (Pe/2-1)*T[i+1] - (1+Pe/2)*T[i-1] );
    } 
//RESIDUAL
    for( int i=1 ; i<n-1 ; ++i){
        res[i]= (-1-Pe/2)*T[i-1] + 2*T[i] + (-1+Pe/2)*T[i+1];
        // cout<<" res[i] "<<res[i]<<endl;
        res_avg += pow(res[i],2)/(n-2); 
    }
    res_avg += pow(res_avg,0.5); 
    // cout<<"res_avg"<<res_avg<<endl;
    ++iter;
...
E[0] = 1 - Pe/2; D[0] = -2;    W[n-3] = 1 + Pe/2; D[n-3] = -2;
Q[n-3] = Pe/2-1; 

for(int i=1 ; i<n-2 ; ++i){
  W[i]= 1 + Pe/2  ; D[i]=-2 ; E[i] = 1 - Pe/2;
}
