...
while(log10(res_max)>tol){
    res_max=0;                     //INITIALISING MAX RESIDUAL FOR THE RESPECTIVE ITERATION
    double err=0;double res_avg=0;// INITIALISING AVERAGE ERROR AND AVERAGE RESIDUAL 
    for(int i=1;i<ny-1;++i){
        for(int j=1;j<nx-1;++j){
            double dummy=0.25*( 4*(1-w)*T[i][j]+T[i][j+1]+T[i][j-1]+T[i+1][j]+T[i-1][j]-dx*dy*(S[i][j]));
